...
def newOdom (msg):
	...
	x=msg.pose.pose.position.x*0.845/20.0 + pointList[1]
	y=msg.pose.pose.position.y*1.08/26.0 + pointList[0]
...
while not rospy.is_shutdown():
	if listnum < listlength:
		goal_x = pointList[listnum+1]
		goal_y = pointList[listnum]
	...
	pid_theta = thetaPID()
	#print("***", listnum, "/ ", listlength, "*** x : ", x, "y : ", y)
	if (err_theta > 0.3):
