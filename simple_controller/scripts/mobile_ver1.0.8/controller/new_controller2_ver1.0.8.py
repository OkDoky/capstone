...
r_size_y = size_y/1.525
threshold = 3.5					### distance error is threshold * 2.095/size_x
yaw = 0.0
yaw_temp = 0.0
r2d = 180.0/pi
robot_path = []
...
def plot_path(path, robot_path):
	...
	area1.set_title('RED - compare real path and path list')
...
def newOdom (msg):
	...
	if listnum == listlength and flag == 2:
		...
		if ((err_theta_d > 3 and err_theta_d < 360) or (err_theta_d < -3 and err_theta_d > -360)):
			print(" if err_theta_d",err_theta_d)
			speed.linear.x = 0.0
			if(err_theta_d<0):
				print(" < 0",err_theta_d)
...
##########  after get pointList  ##############
def speed_publisher(pid_theta, inc_x, inc_y):
	global r2d
	global flag
	global err_theta
	...
	if ((err_theta_d > 3 and err_theta_d < 360) or (err_theta_d < -3 and err_theta_d > -360)):
		...
		speed.linear.x = 0.05
		if(err_theta_d<0):
			speed.angular.z = pid_theta*0.04
		else:
			speed.angular.z = -pid_theta*0.04
	pub.publish(speed)
...
def start():
	...
	if flag == 1:
		...
		speed_publisher(pid_dist, inc_x, inc_y)
...
def direction():
	global pointList
	end_point = pointList[listlength]
	width = 120
