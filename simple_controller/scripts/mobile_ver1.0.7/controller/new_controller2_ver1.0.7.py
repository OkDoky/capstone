...
threshold = 0.6					### distance error is threshold * 2.095/size_x
yaw = 0.0
yaw_temp = 0.0
r2d = 180.0/pi
robot_path = []
...
def newOdom (msg):
	...
	if listnum == listlength and flag == 2:
		...
		if ((err_theta_d > 3 and err_theta_d < 360) or (err_theta_d < -3 and err_theta_d > -360)):
			speed.linear.x = 0.0
			if(err_theta_d<0):
				speed.angular.z = -pid_theta*0.35
				print('z :',speed.angular.z, 'err theta :', err_theta_d)
			else:
				speed.angular.z = pid_theta*0.35
		else:
			speed.angular.z = 0.0
